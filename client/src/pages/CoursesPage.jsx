...
export default function CoursesPage() {
	...
	return (
		<div className="space-y-6">
			<form onSubmit={addCourse} className="bg-slate-800/60 border border-slate-700 rounded-xl p-4 grid md:grid-cols-5 gap-3">
			...
			{loading ? (
				...
				<div className="grid gap-3">
					{courses.map(c => (
						<div key={c.id} className="bg-slate-800/60 border border-slate-700 rounded-xl p-4 flex items-center justify-between gap-4">
							<div className="min-w-0">
								<div className="font-semibold"><Link to={`/courses/${c.id}`} className="hover:text-brand">{c.title}</Link></div>
								<div className="text-sm text-slate-400">المستويات: {c.total_levels ?? 6} — المحاضرات المنجزة: {c.lectures_done ?? 0} — ليفل: {c.level ?? 1} — التقدم: {c.progress ?? 0}%</div>
						...
						<div className="flex items-center gap-2 shrink-0">
							<button onClick={()=> updateCourse(c.id, { lectures_done: (c.lectures_done ?? 0) + 1 })} className="px-3 py-1 rounded bg-slate-700 hover:bg-slate-600">+ محاضرة</button>
							<button onClick={()=> updateCourse(c.id, { lectures_done: Math.max(0, (c.lectures_done ?? 0) - 1) })} className="px-3 py-1 rounded bg-slate-700 hover:bg-slate-600">- محاضرة</button>
							<button onClick={()=> updateCourse(c.id, { total_levels: (c.total_levels ?? 6) + 1 })} className="px-3 py-1 rounded bg-slate-700 hover:bg-slate-600">+ ليفل إجمالي</button>
							<button onClick={()=> updateCourse(c.id, { total_levels: Math.max(1, (c.total_levels ?? 6) - 1) })} className="px-3 py-1 rounded bg-slate-700 hover:bg-slate-600">- ليفل إجمالي</button>
							<button onClick={()=> removeCourse(c.id)} className="px-3 py-1 rounded bg-rose-600 hover:bg-rose-500">حذف</button>
						</div>
					</div>
					))}
				</div>
