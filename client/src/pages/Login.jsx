	...
	return (
		<AuthLayout title="مرحبًا بعودتك" subtitle={<Typewriter messages={["اشتقنالك! لنكمل الإنجاز.", "جاهز لمغامرة تعلم جديدة؟", "ابدأ من حيث توقفت ✨"]} />}>
			<form onSubmit={onSubmit} className="space-y-3 sm:space-y-4">
				...
				<button disabled={loading} className="w-full px-3 sm:px-4 py-2.5 sm:py-3 rounded-xl bg-brand hover:bg-brand-dark text-base sm:text-lg">{loading ? '...جارٍ الدخول' : 'دخول'}</button>
			</form>
			<div className="text-center text-xs sm:text-sm text-slate-400 mt-3 sm:mt-4">جديد هنا؟ <Link to="/register" className="text-brand hover:underline">انضم الآن</Link></div>
		</AuthLayout>
	)
} 